...
struct
  ...
  fun linearize(stm0: T.stm) : T.stm list =
      let
          ...
          fun reorder ((e as T.CALL _ )::rest) = let val t = Temp.newtemp()
            ...
            | reorder (a::rest) = let val (stms,e) = doExp a
          ...
          and reorderExp(el,build) = let val (stms,el') = reorder el
                                     in (stms, build el')
                                     end

          and reorderStm(el,build) = let val (stms,el') = reorder (el)
                                     in stms % build(el')
                                     end

          and doStm(T.SEQ(a,b)) = doStm a % doStm b
            | doStm(T.JUMP(e,labs)) = reorderStm([e],fn [e] => T.JUMP(e,labs))
            | doStm(T.CJUMP(p,a,b,t,f)) = reorderStm([a,b], fn[a,b]=> T.CJUMP(p,a,b,t,f))
            | doStm(T.MOVE(T.TEMP t,T.CALL(e,el))) = reorderStm(e::el,fn e::el => T.MOVE(T.TEMP t,T.CALL(e,el)))
            | doStm(T.MOVE(T.TEMP t,b)) = reorderStm([b],fn[b]=>T.MOVE(T.TEMP t,b))
            | doStm(T.MOVE(T.MEM e,b)) = reorderStm([e,b],fn[e,b]=>T.MOVE(T.MEM e,b))
            | doStm(T.MOVE(T.ESEQ(s,e),b)) = doStm(T.SEQ(s,T.MOVE(e,b)))
            | doStm(T.EXP(T.CALL(e,el))) = reorderStm(e::el,fn e::el => T.EXP(T.CALL(e,el)))
            | doStm(T.EXP e) = reorderStm([e],fn[e]=>T.EXP e)
            | doStm s = reorderStm([],fn[]=>s)
                        
          and doExp(T.BINOP(p,a,b)) = reorderExp([a,b], fn[a,b]=>T.BINOP(p,a,b))
            | doExp(T.MEM(a)) = reorderExp([a], fn[a]=>T.MEM(a))
            | doExp(T.ESEQ(s,e)) = let val stms = doStm s
                                       val (stms',e) = doExp e
                                   in (stms % stms',e)
                                   end
            | doExp(T.CALL(e,el)) = reorderExp(e::el, fn e::el => T.CALL(e,el))
  ...
  fun trace(table,b as (T.LABEL lab :: _),rest) = 
      ...
      in case splitlast b
          of (most,T.JUMP(T.NAME lab, _)) => (case Symbol.look(table, lab)
           ...
           | (most,T.CJUMP(opr,x,y,t,f)) => (case (Symbol.look(table,t), Symbol.look(table,f))
                                              of (_, SOME(b' as _::_)) => b @ trace(table, b', rest)
                                               | (SOME(b' as _::_), _) => most @ [T.CJUMP(T.notRel opr,x,y,f,t)]
                                                                               @ trace(table, b', rest)
  ...
  and getnext(table,(b as (T.LABEL lab::_))::rest) = (case Symbol.look(table, lab)
                                                       of SOME(_::_) => trace(table,b,rest)
                                                        | _ => getnext(table,rest))
    | getnext(table,nil) = nil

  fun traceSchedule(blocks,done) = getnext(foldr enterblock Symbol.empty blocks, blocks)
                                   @ [T.LABEL done]
